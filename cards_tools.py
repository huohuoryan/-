...
def show_all():
    ...
    if len(card_list)==0:
        print('当前没有名片记录，请输入名片内容')
        #return下方的代码不会被执行
        #如果return 后面没有任何的内容，返回到调用表头的位置
        return
    #打印表头
...
def deal_card(find_dict):
    ...
    if action_str == '1':
        ...
        find_dict['phone'] = input_card_info(find_dict['phone'],'修改phone:')
...
def input_card_info(dict_value: object, tip_message: object) -> object:
    """输入名片信息

    :param dict_value:字典中原有的值
    :param tip_message:输入的提示文字
    :return:如果用户输入内容就返回内容，否则返回原有的值
    """
    #1,提示用户输入内容
